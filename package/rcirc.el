(use-package rcirc
  ...
  ;; from spacemacs:
  ;; spacemacs/contrib/!irc/rcirc/funcs.el

  (defun personal/rcirc-authinfo-config ()
    "Initialize authinfo.
...
This doesn't support the chanserv auth method. "
  ...
  (defun personal/znc-auth-source-fetch-password (server)
  ...
  (defun personal/znc-rcirc-server-alist-get-authinfo (server-alist)
 ...
 from .authinfo.gpg"
    (dolist (server server-alist server-alist)
      (let* ((host  (car server))
             ...
             (pass  (personal/znc-auth-source-fetch-password server)))
        (when auth
  ...
  (defun personal/znc-rcirc-connect ()
    ...
    (loop
     ...
     (destructuring-bind (&key host
       ...
       (let ((host (or host server)) ; catter with server without :host
         ...
         (unless connected
           (message "ZNC: host '%s' port '%d' nick '%s' user-name '%s' full-name '%s' password '%s' encryption '%s'" host port nick user-name full-name password (symbol-name encryption))
           (let ((process
